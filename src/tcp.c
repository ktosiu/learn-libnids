...
//ECN
    ...
    {
		/* 是新的TCP流且该报文是第一次握手报文则创建一个新的TCP流结构 */
	...
	/* TCP连接第二次握手 来自于服务端的报文SYN,ACK*/
    if ((this_tcphdr->th_flags & TH_SYN))
    {
		/* 来自客户端的握手包或状态异常的数据包直接丢弃 */
    ...
    if (
        ! (  !datalen && ntohl(this_tcphdr->th_seq) == rcv->ack_seq  )	/* 该报文有负载或无负载且确认号不等于发送序列号 */
        &&
        ( !before(ntohl(this_tcphdr->th_seq), rcv->ack_seq + rcv->window*rcv->wscale) ||	/* 该段报文在接收端窗口以外 */
          before(ntohl(this_tcphdr->th_seq) + datalen, rcv->ack_seq)		/* 该段报文已经被确认 */
        )
    )
        return;
